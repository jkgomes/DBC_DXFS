...
//static const char* PREFERRED_CIPHERS = "AES128-SHA256:AES256-SHA256:AES128-GCM-SHA256:AES256-GCM-SHA384";
		...
		**/
		int tcpsslsinit(SOCKET sd, BIO * certinput /*char *certificate*/)
		{
			...
			if (!sslsock->ctx) {
				...
				strcat(tcperrorstring, "TLS_server_method()");
				strcat(tcperrorstring, "):call failed");
				if (ssllog) fprintf(logfile, "%s", tcperrorstring);
				return -1;
			}
			...
			if (SSL_CTX_use_certificate_file(sslsock->ctx, "cert.pem", SSL_FILETYPE_PEM) <= 0) {
				ERR_print_errors_fp(stderr);
				return -1;
			}

			if (SSL_CTX_use_PrivateKey_file(sslsock->ctx, "key.pem", SSL_FILETYPE_PEM) <= 0) {
				ERR_print_errors_fp(stderr);
				return -1;
			}
